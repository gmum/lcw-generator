...
class FidComputer:
    ...
    def compute_for_images(self, images: torch.Tensor, device: str):
        inception_model = self.__inception_model.to(device)
        with torch.no_grad():
            return self.__compute_for_images(inception_model, images)
...
class FidEvaluator:
    ...
    def evaluate_images(self, images: torch.Tensor, device: str):
        fid_score = self.__fid_computer.compute_for_images(images, device)
        return fid_score
